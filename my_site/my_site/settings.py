...
SECRET_KEY = '))jl0v-%mqh-zt87(7#ot59k8u)qov2=y@)jja$k(p*5u!ipb6'
...
TEMPLATES = [
    {
        ...
        'DIRS': [os.path.join(BASE_DIR, 'templates')],  # добавляем путь к папке templates для базового шаблона
        'APP_DIRS': True,
        'OPTIONS': {
            'context_processors': [
...
LANGUAGE_CODE = 'ru'  # тут меняем формат вывода времени привычный нам с 'en-us'(Американского представления) на 'ru'
...
# дописываем константы для поля фото и сохранения этих фото
...
CKEDITOR_UPLOAD_PATH = "uploads/"  # настройка редактора вадминке
# либо кастомная настройка редактора
CKEDITOR_CONFIGS = {
    'default': {
        'skin': 'moono',
        # 'skin': 'office2013',  # почемуто не работает
        # 'skin': 'flat',  # почемуто не работает
        'toolbar_Basic': [
            ['Source', '-', 'Bold', 'Italic']
        ],
