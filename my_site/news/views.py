...
def login(request):  # ф-я для логина и заносим её в маршруты урл.пй
    return render(request, 'news/login.html')  # и после не забываем создать шаблон(login.html) для ф-и для ф-и в templates/news
