...
urlpatterns = [
    ...
    path('login/', login, name='login'),  # путь для регистра, при указ.маршруте 'login/' вызыв ф-ю login из views
    ...
    path('test/', test, name='test'),  # тестируем построчную пагинацию, когда будет запрошен адрес test/, мы обратимся к фу-ци test
]
